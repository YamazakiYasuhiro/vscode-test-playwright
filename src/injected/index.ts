...
class VSCodeTestServer {
  ...
  private _handleMessage({ op, id, data }: RequestMessage) {
    switch (op) {
      ...
      case "invokeMethod":
        this._invokeMethod(
          id,
          data as MessageRequestDataMap["invokeMethod"]
        ).catch(() => {});
    }
  }
  ...
  private async _invokeMethod(
    id: number,
    {
      ...
      params,
      returnHandle,
    }: MessageRequestDataMap["invokeMethod"]
  ) {
  ...
  private _release(
    id: number,
    { objectId, dispose }: { objectId?: number; dispose?: boolean }
  ) {
  ...
  private _fromParam(param: any): any {
    ...
    return Object.fromEntries(
      Object.entries(param).map(([k, v]) => [k, this._fromParam(v)])
    );
  }
